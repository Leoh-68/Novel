...
function doEnter22(evt, obj, url) {
	if (url == '') {
		notifyDialog('Đường dẫn không hợp lệ');
		return false;
	}
...
function addCoin(_root) {

	let formData = new FormData(_root);
	let idMember = formData.get('id');
	...
	$.ajax({
		url: 'add-coin',
		type: 'GET',
		data: { id: idMember, coin: coin, id_user: idUser },
		success: function (data) {
			data = JSON.parse(data);
			if (data.status == 1) {
				Swal.fire({
					title: "Thành công!",
					text: "Thêm coin thành công!",
					icon: "success"
				});
				console.log(this.showAddCoin);
				
				$('#coin-' + idMember).html(data.newCoin);
			} else {
				Swal.fire({
					title: "Thất bại!",
				...
				$('#wrap-add-coin').css('display', 'none');
			}
			holdonClose();
		}
	});
...
$(document).ready(function () {
	...
	/* Copy */
	if ($('#dropdownCopyMember').length) {
		$('body').on('click', '#dropdownCopyMember', function () {
			var id = $(this).attr('data-id');
			var table = $(this).attr('data-table');
			var com = $(this).attr('data-com');
			var type = $(this).attr('data-type');
