...
class Func
{
    ...
    public function getFollowedChapterUpdate($idMember = 0, $idProduct = 0, $type = 'follow')
    {
        return FollowModel::where('id_member', $idMember)->where('id_product', $idProduct)->where('type', $type)->first();
    }   
    public function checkFollow($idMember = 0, $idProduct = 0, $type = 'follow')
    {
        $checkFollow = FollowModel::where('id_member', $idMember)->where('id_product', $idProduct)->where('type', $type)->first();
    ...
    public function getMember($idMember = 0)
    {
    ...
    public function getTypeMember($type = 'reader')
    {
        switch ($type) {    
            case 'reader':
                return 'Đọc giả';
    ...
    public function getNovel($novel = 0)
    {
        return ProductModel::select('*')
            ->where('id', $novel)
            ->first();
    ...
    public function getChapterFromNovel($idNovel = 0, $type = 'chuong-truyen')
    {
        return ProductModel::select('*')
            ->where('id_novel', $idNovel)
            ->where('type', $type)
            ->whereRaw("FIND_IN_SET(?,status)", [ 'hienthi' ]);
    }


    public function counterProductMember($idMember = 0, $type = 'truyen')
    {
        return ProductModel::where('id_member', $idMember)->where('type', $type)->count();
    }



    public function getMemberPlace($idMember = 0, $type = 'phan-khu')
    {
        return NewsModel::select('*')
            ->where('type', $type)
            ->whereRaw("FIND_IN_SET(?,member_list)", [ $idMember ])
            ->get();
    }
...
*/
    ...
    public function getLinkCategory($table = '', $level = '', $type = '', $title_select = '')
    {
        ...
        $query = DB::table($table)->select('namevi', 'id')
            ->where('type', $type);
        if (!empty($idlist))
            $query->where('id_list', $idlist);
        if (!empty($idcat))
            $query->where('id_cat', $idcat);
        if (!empty($iditem))
        ...
        if (!empty($rows)) {
            foreach ((array) json_decode($rows, true) as $v) {
                if (isset(Request()->$id_parent) && ($v["id"] == (int) Request()->$id_parent))
                    $selected = "selected";
                else
                    $selected = "";
    ...
    public function getLinkCategoryMember($table = '', $level = '', $type = '', $title_select = '')
    {
        ...
        $str = '<select id="' . $id_parent . '" name="' . $id_parent . '" class="form-control filter-category select2"><option value="0">' . $title_select . '</option>';
        if (!empty($rows)) {
            foreach ((array) json_decode($rows, true) as $v) {
                if (isset(Request()->$id_parent) && ($v["id"] == (int) Request()->$id_parent))
                    $selected = "selected";
    ...
    public function getAjaxCategory($table = '', $table_child = '', $level = '', $type = '', $title_select = 'Chọn danh mục', $class_select = 'select-category')
    {
        ...
        foreach ((array) json_decode($rows, true) as $v) {
            if (request()->$id_parent && ($v["id"] == (int) request()->$id_parent))
                $selected = "selected";
            else
                $selected = "";
    ...
    public function getAjaxCategoryGroup($table = '', $table_child = '', $level = '', $type = '', $title_select = 'Chọn danh mục', $class_select = 'select-category')
    {
        ...
        if (!empty($tableParent)) {
            $query = DB::table($tableParent)->select('namevi', 'id')
                ->where('type', $type);
            if (!empty($idlist))
                $query->whereIn('id', explode(',', $idlist));
            if (!empty($idcat))
                $query->whereIn('id', explode(',', $idcat));
            if (!empty($iditem))
                $query->whereIn('id', explode(',', $iditem));
            ...
            foreach ((array) json_decode($rows, true) as $v) {
                ...
                foreach ($v['cats'] as $value) {
                    if (request()->$id_parent && (in_array($value["id"], $check)))
                        $selected = "selected";
                    else
                        $selected = "";
            ...
            $query = DB::table($table)->select('namevi', 'id')
                ->where('type', $type);
            if (!empty($idlist))
                $query->whereIn('id_list', [ $idlist ]);
            $rows = $query->get();

            $str = '<select id="' . $id_parent . '" name="data[' . $id_parent . '][]"  data-url="category-group" ' . $data_level . ' ' . $data_type . ' ' . $data_table . ' ' . $data_child . ' class="form-control select2 selectGroup ' . $class_select . '" multiple="multiple">';
            foreach ((array) json_decode($rows, true) as $v) {
                if (request()->$id_parent && (in_array($v["id"], $check)))
                    $selected = "selected";
                else
                    $selected = "";
    ...
    public function orderStatus($status = 0)
    {
        ...
        foreach ($row as $v) {
            if (isset(Request()->order_status) && ($v["id"] == (int) Request()->order_status) || ($v["id"] == $status))
                $selected = "selected";
            else
                $selected = "";
    ...
    function orderPayments()
    {
        ...
        foreach ($row as $v) {
            if (isset(Request()->order_payment) && ($v["id"] == (int) Request()->order_payment))
                $selected = "selected";
            else
                $selected = "";
            $str .= '<option value=' . $v["id"] . ' ' . $selected . '>' . $v["namevi"] . '</option>';
