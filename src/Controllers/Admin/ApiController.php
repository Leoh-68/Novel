...
class ApiController
{
    ...
    public function addCoin(Request $request)
    {
        ...
        $coin = $request->coin;
        if (!empty($member)) {
            $member->coin = (int)$member->coin + (int)$coin;
            $member->save();
            UserActionHistory::create([
                ...
                'number_coin' => $coin,
                'type' => 'addbyadmin',
                'contentvi' => 'Thêm ' . $coin . ' coin cho ' . $member->fullname,
            ]);
            echo json_encode(['status' => 1, 'newCoin' => $member->coin]);
        }
        else{
            echo json_encode(['status' => 0, 'newCoin' => $member->coin]);
        }
    }
    ...
    public function copy(Request $request)
    {
        ...
        if (!empty($row)) {
            ...
            if (!empty($row['id_member'])) {
                $dataCopy['id_member'] = $row['id_member'];
            }
            if (!empty($row['id_cat'])) {
