...
class NewsController
{
    ...
    public function edit($com, $act, $type, Request $request)
    {
        ...
        $member = [];
        if (!empty($this->configType->$type->member)) {
            $member = MemberModel::select('*')
                ->whereRaw('find_in_set(?,status)', [ 'hienthi' ])
                ->orderBy('numb', 'asc')
                ->get();
        }
        return view('news.man.add', [ 'item' => $item, 'gallery' => $gallery, 'id_parent' => $id_parent, 'member' => $member ]);
    }
            ...
            if ($data) {
                ...
                if (!empty($this->configType->$type->member)) {
                    if (!empty($data['member_list'])) {
                        $data['member_list'] = !empty($data['member_list']) ?
                        implode(",", $data['member_list']) : '';
                    }
                }
