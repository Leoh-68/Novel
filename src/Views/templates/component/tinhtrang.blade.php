...
<div class="card-body">
                ...
                <div class="form-group d-inline-block mb-2 me-5">
                    ...
                    <label class="switch switch-success">
                        <input type="checkbox" name="status[{{ $key }}]"
                            class="switch-input custom-control-input show-checkbox" id="{{ $key }}-checkbox"
    ...
    @if (!empty($color))
        <div class="form-group last:!mb-0">
            <label for="color" class="d-inline-block align-middle mb-0 mr-2 form-label">Màu:</label>
            <input type="color"
                class="form-control form-control-mini w-25 text-left d-inline-block align-middle text-sm" min="0"
                  name="data[color]" id="color" placeholder="Màu"
                value="{{ isset($item['color']) ? $item['color'] : 1 }}">
        </div>
    @endif
    {!! $slot !!}
</div>
